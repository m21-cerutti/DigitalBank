...
test('activation/désactivation du switch 2FA', async () => {
  await bootApp();
  ...
  document.querySelector('[data-testid="tab-security"]').click();
  await new Promise((r) => setTimeout(r, 20));

  const toggle = document.querySelector('#toggle-2fa');
  toggle.checked = !toggle.checked;
  toggle.dispatchEvent(new Event('change', { bubbles: true }));
  await new Promise((r) => setTimeout(r, 10));

  const success = document.querySelector('[data-testid="security-success"]');
